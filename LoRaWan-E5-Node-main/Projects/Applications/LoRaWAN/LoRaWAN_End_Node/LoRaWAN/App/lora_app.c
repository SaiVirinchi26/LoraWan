...
//        }
    	  ...
    	  APP_LOG(TS_ON, VLEVEL_L, "@@@@@@@ Rx_data  : ~%x\r\n",appData->Buffer[i]);
...
//  int16_t temperature = 0;
 // sensor_t sensor_data;
  uint16_t Tx_temp=0,Tx_press=0,Tx_humi=0,Battery=5000,mode=1;
  UTIL_TIMER_Time_t nextTxIn = 0;
  uint8_t Temp_val[7],Press_val[7],Humi_val[7];
  int value=BME280_Measure();
  Tx_temp=(Temperature)*100;
  Tx_press=(Pressure)/1000*100; // conversion into KiloPascal
  Tx_humi=(Humidity)*100;

  if(value==0)
  {
	  gcvt(Temperature,7,Temp_val);
	  APP_LOG(TS_ON, VLEVEL_L, "@@@@@@@ sensor_Temp  : ~%s\r\n",Temp_val);
	  gcvt(Pressure,7,Press_val);
	  APP_LOG(TS_ON, VLEVEL_L, "@@@@@@@ sensor_press  : ~%s\r\n",Press_val);
	  gcvt(Humidity,7,Humi_val);
	  APP_LOG(TS_ON, VLEVEL_L, "@@@@@@@ sensor_Humi  : ~%s\r\n",Humi_val);
  }
...
//  pressure    = (uint16_t)(sensor_data.pressure * 100 / 10);      /* in hPa / 10 */
  ...
  AppData.Buffer[i++] = (uint8_t)((Tx_humi >> 8) & 0xFF);
  AppData.Buffer[i++] = (uint8_t)(Tx_humi & 0xFF);
